...
export default function HackTeamUp() {
  const [teams, setTeams] = useState<HackTeamBrief[]>([]);

  const { data: session } = authClient.useSession();

  const [scopeFilter, setScopeFilter] = useState<TeamScope>("ALL");
  ...
  async function fetchTeams(query?: string) {
    try {
      const url = query ? `/api/hack-team?${query}` : "/api/hack-team";
  ...
  async function handleApplyFilters(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (hackModeFilter) params.set("mode", hackModeFilter);
    if (teamSizeFilter && teamSizeFilter.value !== null)
    ...
    setShowFilter(false);
    router.push(`/main/hacks-teamup?${params.toString()}`, { scroll: false });
  }
  ...
  const hasActiveFilters =
    Boolean(hackModeFilter) ||
    Boolean(teamSizeFilter && teamSizeFilter.value !== null) ||
      ...
      <>
        <div className="flex items-center justify-between mt-15 sm:mt-20 font-inter max-w-7xl mx-auto">
          <div className="relative inline-block">
            ...
            shadow-[0_0_12px_rgba(79,241,241,0.4)]"
            />
          </div>
