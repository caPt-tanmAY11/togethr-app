...
export default function OnboardingStep3() {
  ...
  const cachedData = queryClient.getQueryData<OnboardingMeResponse>(["onboarding-me"]);

  const [education, setEducation] = useState<EducationForm[]>(
    cachedData?.education && cachedData.education.length > 0 
  ...
  useEffect(() => {
    if (freshData?.education && freshData.education.length > 0) {
      setEducation(freshData.education.map(edu => ({
  ...
  const { mutate: submitStep3, isPending: loading } = useMutation({
    mutationFn: async (educationData: EducationForm[]) => {
  ...
  function updateField(index: number, field: keyof EducationForm, value: string) {
    setEducation((prev) =>
      ...
      >
        <div className="mb-5 sm:mb-6">
          <div className="flex items-center justify-between gap-4">
            <span className="text-[10px] sm:text-xs tracking-widest text-white/60">STEP 3 OF 4</span>
            <span className="text-[10px] sm:text-xs text-white/40">Education</span>
          </div>
        ...
        <div className="mb-5 sm:mb-6">
          ...
          <p className="text-xs sm:text-sm text-white/60 mt-1">Add your academic background</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                  <input
                    ...
                    className="rounded-lg bg-white/5 border border-white/10 px-2 sm:px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm outline-none focus:border-white/30"
                  />
                  <input
                    placeholder="Field of study"
                    value={edu.fieldOfStudy || ""}
                    onChange={(e) => updateField(index, "fieldOfStudy", e.target.value)}
                    className="rounded-lg bg-white/5 border border-white/10 px-2 sm:px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm outline-none focus:border-white/30"
                  />
                  <input
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-lg bg-white/5 border border-white/10 px-2 sm:px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm outline-none resize-none focus:border-white/30"
                />
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs sm:text-sm text-white/70 hover:text-white transition"
          >
