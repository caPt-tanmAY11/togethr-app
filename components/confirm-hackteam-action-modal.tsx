...
interface ConfirmHackTeamActionModalProps {
  open: boolean;
  title: string;
  description: string;
  confirmText: string;
...
export default function ConfirmHackTeamActionModal({
  open,
  title,
  description,
  confirmText,
  ...
  onConfirm,
  onClose,
}: ConfirmHackTeamActionModalProps) {
  if (!open) return null;
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div className="w-full max-w-md rounded-2xl bg-[#00000080] border border-white/10 shadow-xl p-6 text-white">
